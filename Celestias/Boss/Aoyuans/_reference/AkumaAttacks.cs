...
namespace AAMod
{
    public class AkumaAttacks
	{
        public static void Dragonfire(NPC npc, Mod mod, bool Awakened = false)
        {
            ...
            int ProjectileType = Awakened ? ModContent.ProjectileType<AkumaAMeteor>() : ModContent.ProjectileType<AkumaMeteor>();
            Projectile.NewProjectile(vector2.X, vector2.Y, 0, num115 * 1.5f, ProjectileType, npc.damage / 4, 0, player.whoAmI, 0f, 0.5f + ((float)Main.rand.NextDouble() * 0.3f));
        }
