...
namespace AncientChineseMythology.Celestias.Boss.Aoshuns
{
    ...
    /// 纹理AoshunBody.png: 112×320, 5帧, 每帧112×64
    /// ai[1]: 前一段NPC索引
    /// ai[2]: 当前段使用的帧号（0-4）
    ...
    public class AoshunBody : ModNPC
    {
        ...
        public override bool PreAI() {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient) {
                if (!Main.npc[(int)NPC.ai[1]].active || Main.npc[(int)NPC.ai[3]].type != ModContent.NPCType<Aoshun>()) {
                    NPC.life = 0;
        ...
        public override bool CheckActive() {
            if (NPC.AnyNPCs(ModContent.NPCType<Aoshun>()))
                return false;
            NPC.active = false;
            return true;
        }
