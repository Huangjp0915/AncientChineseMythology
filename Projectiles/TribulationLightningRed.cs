...
{
    public class TribulationLightningRed : ModProjectile
    {
        ...
        private readonly List<LightTreeRed> trees = new();
        ...
        public override void AI() {
            ...
            for (int i = 0; i < Main.maxPlayers; i++) {
                ...
                // 每名玩家生成一棵树状闪电
                LightTreeRed t = new LightTreeRed(Main.rand, origin, Projectile.originalDamage, plr);
    ...
    public class LightTreeRed
    {
        ...
        public LightTreeRed(UnifiedRandom random, Vector2 position, float damage, Player owner) {
            cnt = 0;
            root = null;
            this.random = random;
        ...
        private void CreateExplosionParticles(Vector2 center) {
            ...
            for (int i = 0; i < particleCount; i++) {
                ...
                // 创建粒子
                Dust dust = Dust.NewDustPerfect(center, DustID.CrimsonTorch, velocity, 150, new Color(255, 60, 60), 1.4f);
        ...
        private void CreateTerraParticle(Vector2 position) {
            // 创建 Terra 粒子
            Dust dust = Dust.NewDustPerfect(position, DustID.CrimsonTorch, new Vector2(Main.rand.NextFloat(-1f, 1f), Main.rand.NextFloat(-1f, 1f)), 100, new Color(255, 60, 60), 1f);
            dust.noGravity = true; // 禁用重力
            dust.fadeIn = 1f;      // 设置淡入效果
        ...
        private void _draw(SpriteBatch sb, Vector2 pos, Vector2 vel, Node node, Vector2 mousePos) {
            ...
            for (float i = 0; i <= node.length; i += 0.04f) {
                sb.Draw(TextureAssets.BlackTile.Value, pos + unit * i + new Vector2(-30, 0), new Rectangle(0, 0, 1, 2), new Color(255, 60, 60), 0,
